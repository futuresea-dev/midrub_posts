...
namespace MidrubBase\Admin\Components\Collection\Frontend\Helpers;
...
class Classification
{
                                    ...
                                    if ( ($field['slug'] === $classification_slug) && ($field['type'] === 'contents_classification') ) {
                                        ...
                                        if ( isset($field['words_list']['search_input_placeholder']) && isset($field['words_list']['new_classification_option']) && isset($field['words_list']['classification_slug_input_placeholder']) && isset($field['words_list']['classification_name_input_placeholder']) && isset($field['words_list']['single_item']) ) {
                                            ...
                                            $inputs = '<div class="row">'
                                                . '<div class="col-xs-12">'
                                                    . '<input type="text" class="form-control classification-input" placeholder="' . $field['words_list']['classification_name_input_placeholder'] . '" data-meta="name">'
                                                . '</div>'
                                            . '</div>';
                                            ...
                                            if ( isset( $field['fields'][0]['slug'] ) ) {
                                                ...
                                                foreach ( $field['fields'] as $cfield ) {
                                                    ...
                                                    if ( $cfield['type'] === 'text' ) {
                                                        ...
                                                        $inputs .= '<div class="row">'
                                                            . '<div class="col-xs-12">'
                                                                . '<input type="text" class="form-control classification-input" placeholder="' . $cfield['placeholder'] . '" data-meta="' . $cfield['slug'] . '">'
                                                            . '</div>'
                                                        . '</div>';
    ...
    public function create_new_classification_item()
    {
        ...
        if ($this->CI->input->post()) {
            ...
            if ($this->CI->form_validation->run() === false) {
                ...
                $params = array(
                    'slug' => $meta_option_classification_slug,
                ...
                if ( $parent ) {
                    ...
                    $check_parent = $this->CI->base_model->get_data_where('classifications', 'classification_id', array(
    ...
    public function get_classification_parents()
    {
        ...
        if ($this->CI->input->post()) {
            ...
            if ($this->CI->form_validation->run() !== false) {
                ...
                // Get classifications
                $classifications = $this->CI->base_classifications->get_classifications_by_slug($args);
                ...
                if ( $classifications ) {
                    ...
                    foreach ($classifications as $classification) {
                        ...
                        $classification_items[$classification['parent']][] = $classification;
                    ...
                    foreach( $this->items as $classification ) {
                        ...
                        if ( preg_match("/{$key}/i", $classification['name'] ) ) {
                            ...
                            if ( $classification['parent'] < 1 ) {
                                ...
                                $classification['name'] = $parent . ' &#8594; ' . $classification['name'];
                ...
                if ( $classifications ) {
                    ...
                    if ($this->items) {
                        ...
                        foreach ($this->items as $classification) {
                            ...
                            if ($classification_parent < $classification['parent']) {

                                $all_classifications .= '<ul class="collapse" id="classification-show-collection-' . $classification['parent'] . '">';

                                $count_ul++;

                                $parent_level[$classification['parent']] = $count_ul;

                            } else if ($classification_parent > $classification['parent']) {
                                ...
                                if ($classification['parent'] < 1) {
                                    ...
                                    $minus = $count_ul - $parent_level[$classification['parent']];
                            ...
                            // Set new parent
                            $classification_parent = $classification['parent'];

                            $pa = 0;

                            if ( $classification['parent'] ) {
                                $pa = $parent_level[$classification['parent']];
                            }
                            ...
                            // Add html
                            $all_classifications .= $this->classification_template($classification['name'], $classification['classification_id']);
                ...
                if ( $classifications ) {
                    ...
                    if ($this->items) {
                        ...
                        foreach ($this->items as $classification) {
                            ...
                            if ($classification_parent < $classification['parent']) {
                                $all_classifications .= '<ul class="collapse" id="classification-show-selected-collection-' . $classification['parent'] . '">';
                                $count_ul++;
                                $parent_level[$classification['parent']] = $count_ul;
                            } else if ($classification_parent > $classification['parent']) {
                            ...
                            // Add html
                            $all_classifications .= $this->selected_classification_template($classification['name'], $classification['classification_id'], $all_classification_ids);
    ...
    public function get_content_classifications()
    {
        ...
        if ($this->CI->input->post()) {
            ...
            if ($this->CI->form_validation->run() !== false) {
                ...
                $contents_classifications = $this->CI->base_model->get_data_where(
    ...
    public function order_items($classification_items, $ids=array())
    {
        ...
        if ( $ids ) {
            ...
            foreach ( $items as $item ) {
                ...
                // Verify if parent is 0
                if ( $item['parent'] > 0 && $stop > 0 ) {
                    continue;
                }

                if ( !in_array($item['classification_id'], $selected) && ($item['parent'] < 1) ) {
                    $stop = 1;
                    continue;
                } else if ( in_array($item['classification_id'], $selected) && ($item['parent'] < 1) ) {
                    $stop = 0;
                }
        ...
        . '<div class="row">'
            . '<div class="col-xs-1">'
                . '<div class="checkbox-option-select">'
                    . '<input id="frontent-contents-single-' . $classification_id . '" name="frontent-contents-single-' . $classification_id . '" data-id="' . $classification_id . '" type="checkbox">'
                    . '<label for="frontent-contents-single-' . $classification_id . '">'
                    . '</label>'
                . '</div>'
            . '</div>'
            . '<div class="col-xs-10 clean">'
                . '<a href="#classification-show-collection-' . $classification_id . '" data-toggle="collapse" role="button" aria-controls="classification-show-collection-' . $classification_id . '">'
                    . $classification_name . ' (' . $classification_id . ')'
                . '</a>'
            . '</div>'
            . '<div class="col-xs-1">'
                . '<a href="#" role="button" class="delete-classification-single">'
    ...
    public function selected_classification_template($classification_name, $classification_id, $all_classification_ids)
    {
        ...
        . '<div class="row">'
            . '<div class="col-xs-2">'
                . '<div class="checkbox-option-select">'
                    . '<input id="frontent-contents-selected-' . $classification_id . '" name="frontent-contents-selected-' . $classification_id . '" data-id="' . $classification_id . '" type="checkbox"' . $checked . '>'
                    . '<label for="frontent-contents-selected-' . $classification_id . '">'
                    . '</label>'
                . '</div>'
            . '</div>'
            . '<div class="col-xs-10 clean">'
                . '<a href="#classification-show-selected-collection-' . $classification_id . '" data-toggle="collapse" role="button" aria-controls="classification-show-selected-collection-' . $classification_id . '">'
                    . $classification_name
                . '</a>'
            . '</div>'
        . '</div>';
