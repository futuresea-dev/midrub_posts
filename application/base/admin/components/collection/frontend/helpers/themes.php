...
class Themes {
    ...
    public function upload_theme() {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ( !is_dir(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp') ) {
                mkdir(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp', 0777);
            }

            // Upload the theme
            $config['upload_path'] = MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp';
    ...
    public function unzipping_zip() {
        ...
        if ( is_numeric($this->CI->input->get('file_name')) ) {
            ...
            if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $this->CI->input->get('file_name') . '.zip') ) {

                // Call the ZipArchive class
                $zip = new \ZipArchive;

                // Try to open the zip
                if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $this->CI->input->get('file_name') . '.zip') === TRUE) {

                    // Extract the zip
                    $zip->extractTo(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/');

                    // Close the ZipArchive class
                    $zip->close();

                    // Verify if the installation.json exists
                    if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/installation.json') ) {

                        // Delete the uploaded theme
                        unlink(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $this->CI->input->get('file_name') . '.zip');

                        // Get the installation file
                        $installation = json_decode(file_get_contents(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/installation.json'), true);

                        // Verify if the php file exists
                        if ( isset($installation['php']) ) {

                            // Verify if the php zip file exists
                            if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $installation['php']) ) {

                                // Call the ZipArchive class
                                $zip = new \ZipArchive;

                                // Try to open the zip
                                if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $installation['php']) === TRUE) {
                                        ...
                                        if ( is_dir(MIDRUB_BASE_PATH . 'frontend/themes/' . $zip->getNameIndex(0)) ) {
                                            ...
                                            if ( is_dir(MIDRUB_BASE_PATH . 'frontend/themes/' . $zip->getNameIndex(0)) ) {
                                                ...
                                                if ( isset($installation['assets']) ) {
                                                    ...
                                                    // Verify if the assets file exists
                                                    if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $installation['assets']) ) {

                                                        // Call the ZipArchive class
                                                        $zip = new \ZipArchive;

                                                        // Try to open the zip
                                                        if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/' . $installation['assets']) === TRUE) {
    ...
    public function delete_files() {
        ...
        // specified folder 
        $files = glob(MIDRUB_BASE_ADMIN_COMPONENTS_FRONTEND . 'temp/*');  
