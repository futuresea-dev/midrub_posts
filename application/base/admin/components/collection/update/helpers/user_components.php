<?php
/**
 * User Components Helper
 *
 * This file contains the class User_components
 * with methods to manage the user components updates
 *
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.8.1
...
/*
 * User_components class provides the methods to manage the user components updates
 * 
 * @author Scrisoft
 * @package Midrub
 * @since 0.0.8.1
...
class User_components {
     ...
     * @since 0.0.8.1
     */
    protected $CI, $copied = array(), $component_dir;
    ...
    public function verify() {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ($this->CI->form_validation->run() === false ) {
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {

                    // Set component's dir
                    $this->component_dir = APPPATH . 'base/user/components/collection/' . $slug;
                    ...
                    if ( !empty($component['update_code']) && !$code ) {
                    ...
                    } else if ( !empty($component['update_code']) && $code ) {
                    ...
                    // Get the update
                    $get_update = json_decode(get($component['update_url'] . $update_code), true);
                    ...
                    $data = array(
                        'success' => FALSE,
                        'message' => $this->CI->lang->line('update_component_can_not_be_updated')
                    );
            ...
            if ($this->CI->form_validation->run() === false ) {
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {
                    ...
                    if ( isset($get_update['url']) ) {
                        ...
                        // New Zip name
                        $new_zip = $this->component_dir . '/download.zip';                        
                        ...
                        if ( !is_dir($this->component_dir . '/temp') ) {

                            // Create
                            if ( !mkdir($this->component_dir . '/temp', 0755, true) ) {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ($this->CI->form_validation->run() === false ) {
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {
                    ...
                    // Zip name
                    $zip_name = $this->component_dir . '/download.zip'; 
                    ...
                    if ( $zip->open($zip_name) === TRUE ) {
                        ...
                        // Extract the zip
                        $zip->extractTo($this->component_dir . '/temp/');

                        // Close the ZipArchive class
                        $zip->close();

                        // Verify if the update.json exists
                        if ( file_exists($this->component_dir . '/temp/update.json') ) {

                            // Get files to update
                            $update = json_decode(file_get_contents($this->component_dir . '/temp/update.json'), true);
                            ...
                            if ( isset($update['files']) ) {
                                ...
                                foreach ( $update['files'] as $file ) {
                                    ...
                                    // Verify file exists
                                    if ( !file_exists($this->component_dir . '/temp/' . $file) ) {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ($this->CI->form_validation->run() === false ) {
                ...
                if ( is_dir(APPPATH . 'base/user/components/collection/' . $slug) ) {
                    ...
                    if ( is_dir($this->component_dir . '/backup') ) {

                        $files = new \RecursiveIteratorIterator(
                            new \RecursiveDirectoryIterator($this->component_dir . '/backup', \RecursiveDirectoryIterator::SKIP_DOTS),
                        ...
                        rmdir($this->component_dir . '/backup');
                    ...
                    if ( !is_dir($this->component_dir . '/backup') ) {
                        ...
                        // Create
                        if ( !mkdir($this->component_dir . '/backup', 0755, true) ) {
                    ...
                    if ( file_exists($this->component_dir . '/temp/update.json') ) {
                        ...
                        if ( isset($update['files']) ) {
                            ...
                            foreach ( $update['files'] as $file ) {
                                ...
                                // Backup url
                                $cfile = $this->component_dir . '/backup/' . $file;
                                ...
                                if ( file_exists($file) ) {
                                    ...
                                    if ( count($explode) > 1)  {
                                        ...
                                        // Backup directory
                                        $folder_route = $this->component_dir . '/backup/';
                                    ...
                                    if ( !copy($file, $cfile) ) {
                                        ...
                                        $this->restore_backup($this->component_dir);
                            ...
                            $data = array(
                                'success' => TRUE,
                                'message' => $this->CI->lang->line('update_component_was_updated')
                            );
                            ...
                            if ( file_exists($this->component_dir . '/update.json') ) {

                                // Delete the existing update.json
                                unlink($this->component_dir . '/update.json');

                            }

                            // Copy the update.json in the main directory
                            copy($this->component_dir . '/temp/update.json', $this->component_dir . '/update.json');

                            // Create the update.json in the backup directory
                            file_put_contents($this->component_dir . '/backup/backup.json', json_encode(array(
                                'files' => $this->copied
                            ), JSON_PRETTY_PRINT));

                            // Verify if old download.zip exists
                            if ( file_exists($this->component_dir . '/download.zip') ) {

                                // Delete the existing download.zip
                                unlink($this->component_dir . '/download.zip');

                            }                    

                            // Delete the temp directory
                            $files = new \RecursiveIteratorIterator(
                                new \RecursiveDirectoryIterator($this->component_dir . '/temp', \RecursiveDirectoryIterator::SKIP_DOTS),
                                \RecursiveIteratorIterator::CHILD_FIRST
                            );
     ...
     * 
     * @param string $component_dir contains the component's path 
     * 
     * @since 0.0.8.1
     * 
     * @return void
     */ 
    public function restore_backup($component_dir) {
        ...
        if ( $this->copied ) {
            ...
            foreach ($this->copied as $file) {
                ...
                if ( $file ) {
                    ...
                    // restore it
                    if ( !copy($file, str_replace($component_dir . '/backup/', '', $file)) ) {
        ...
        } else if ( file_exists($component_dir . '/backup/backup.json') ) {

            // Get last version
            $get_last = json_decode(file_get_contents($component_dir . '/backup/backup.json'), TRUE);
            ...
            if ( isset($get_last['files']) ) {
                ...
                foreach ( $get_last['files'] as $file ) {
                    ...
                    if ( file_exists($file) ) {
                        ...
                        // Get file
                        $cfile = str_replace($component_dir . '/backup/', '', $file);
                ...
                if ($error_check == 0) {
                    ...
                    $files = new \RecursiveIteratorIterator(
                        new \RecursiveDirectoryIterator($component_dir . '/backup', \RecursiveDirectoryIterator::SKIP_DOTS),
                        \RecursiveIteratorIterator::CHILD_FIRST
                    );
                    foreach ( $files as $fileinfo ) {
                        $todo = ($fileinfo->isDir() ? 'rmdir' : 'unlink');
                    ...
                    rmdir($component_dir . '/backup');

                    // Verify if old update.json exists
                    if ( file_exists($component_dir . '/update.json') ) {

                        // Delete the existing update.json
                        unlink($component_dir . '/update.json');
...
/* End of file user_components.php */