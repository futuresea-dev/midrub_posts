...
namespace MidrubBase\Admin\Components\Collection\User\Helpers;
...
class Apps {
    ...
    public function upload_app() {
        ...
        if ( $this->CI->input->post() ) {
            ...
            if ( !is_dir(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp') ) {
                mkdir(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp', 0777);
            }

            // Upload the app
            $config['upload_path'] = MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp';
    ...
    public function unzipping_zip() {
        ...
        if ( is_numeric($this->CI->input->get('file_name')) ) {
            ...
            if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $this->CI->input->get('file_name') . '.zip') ) {

                // Call the ZipArchive class
                $zip = new \ZipArchive;

                // Try to open the zip
                if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $this->CI->input->get('file_name') . '.zip') === TRUE) {

                    // Extract the zip
                    $zip->extractTo(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/');

                    // Close the ZipArchive class
                    $zip->close();

                    // Verify if the installation.json exists
                    if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/installation.json') ) {

                        // Delete the uploaded app
                        unlink(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $this->CI->input->get('file_name') . '.zip');

                        // Get the installation file
                        $installation = json_decode(file_get_contents(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/installation.json'), true);

                        // Verify if the php file exists
                        if ( isset($installation['php']) ) {

                            // Verify if the php zip file exists
                            if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['php']) ) {

                                // Call the ZipArchive class
                                $zip = new \ZipArchive;

                                // Try to open the zip
                                if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['php']) === TRUE) {

                                    // First verify if files exists
                                    if ( $zip->numFiles > 0 ) {

                                        // Verify if the app is already installed
                                        if ( is_dir(MIDRUB_BASE_PATH . 'user/apps/collection/' . $zip->getNameIndex(0)) ) {
                                            ...
                                            // Extract the zip
                                            $zip->extractTo(MIDRUB_BASE_PATH . 'user/apps/collection/');

                                            // Verify if models directory exists
                                            if ( is_dir(MIDRUB_BASE_PATH . 'user/apps/collection/' . $zip->getNameIndex(0) . '/models/') ) {

                                                // List all available models
                                                foreach ( glob(MIDRUB_BASE_PATH . 'user/apps/collection/' . $zip->getNameIndex(0) . '/models/*.php') as $filename ) {

                                                    // Get the class's name
                                                    $className = str_replace(array(MIDRUB_BASE_PATH . 'user/apps/collection/' . $zip->getNameIndex(0) . '/models/', '.php'), '', $filename);

                                                    // Load the model
                                                    $this->CI->load->ext_model(MIDRUB_BASE_PATH . 'user/apps/collection/' . $zip->getNameIndex(0) . '/models/', ucfirst($className), $className );
                                            ...
                                            if ( is_dir(MIDRUB_BASE_PATH . 'user/apps/collection/' . $zip->getNameIndex(0)) ) {
                                                ...
                                                if ( isset($installation['assets']) ) {
                                                    ...
                                                    // Verify if the assets file exists
                                                    if ( file_exists(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['assets']) ) {

                                                        // Call the ZipArchive class
                                                        $zip = new \ZipArchive;

                                                        // Try to open the zip
                                                        if ($zip->open(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/' . $installation['assets']) === TRUE) {
    ...
    public function delete_files() {
        ...
        // specified folder 
        $files = glob(MIDRUB_BASE_ADMIN_COMPONENTS_USER . 'temp/*');  
