            ...
            if ( $this->CI->form_validation->run() == false ) {
                ...
                if ( $subscription ) {
                    ...
                    try {
                        ...
                        if ( the_user_option('country', $this->CI->user_id) && the_user_option('state', $this->CI->user_id) && the_user_option('city', $this->CI->user_id) && the_user_option('postal_code', $this->CI->user_id) && the_user_option('street_number', $this->CI->user_id) ) {

                            // Create a Customer:
                            $customer = \Stripe\Customer::create(
                                array(
                                    'name' => the_user_option('first_name', $this->CI->user_id) . ' ' . the_user_option('last_name', $this->CI->user_id),
                                    'address' => array(
                                    'line1' => the_user_option('street_number', $this->CI->user_id),
                                    'postal_code' => the_user_option('postal_code', $this->CI->user_id),
                                    'city' => the_user_option('city', $this->CI->user_id),
                                    'state' => the_user_option('state', $this->CI->user_id),
                                    'country' => the_user_option('country', $this->CI->user_id)
                                    )
                                )
                            );
