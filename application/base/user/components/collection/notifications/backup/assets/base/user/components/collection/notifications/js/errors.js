/*
 * Errors Notifications JavaScript file
*/
...
jQuery(document).ready( function ($) {
    ...
    ********************************/
     ...
     * @since   0.0.8.3
     */
    Main.load_errors_by_page = function (page) {
        
        // Prepare data
        var data = {
            action: 'load_errors_by_page',
            page: page
        };

        // Make ajax call
        Main.ajax_call(url + 'user/component-ajax/notifications', 'GET', data, 'display_errors_by_page_response');
    ...
    ********************************/ 
    ...
    $( document ).on( 'click', '.notifications-page .notifications-delete-alert', function (e) {
        ...
        // Prepare data
        var data = {
            action: 'delete_errors_alert',
            alert: alert_id
        };
        
        // Make ajax call
        Main.ajax_call(url + 'user/component-ajax/notifications', 'GET', data, 'delete_errors_alert_response');
    ...
    ********************************/
     ...
     * @since   0.0.8.3
     */
    Main.methods.display_errors_by_page_response = function ( status, data ) {
        ...
        if ( status === 'success' ) {
            ...
            // List all alerts errors
            for ( var n = 0; n < data.errors.length; n++ ) {

                // Unread mark
                var unread = '';

                // Verify if the errors alert is unread
                if ( data.errors[n].page_seen === null ) {
                    unread = ' class="notifications-unread-notification"';
                }
                
                notifications += '<li data-alert="' + data.errors[n].alert_id + '"' + unread + '>'
                    + '<div class="row">'
                        + '<div class="col-11">'
                            + '<a href="' + url + 'user/notifications?p=errors&alert=' + data.errors[n].alert_id + '" class="notifications-show-notification">'
     ...
     * @since   0.0.8.3
     */
    Main.methods.delete_errors_alert_response = function ( status, data ) {
