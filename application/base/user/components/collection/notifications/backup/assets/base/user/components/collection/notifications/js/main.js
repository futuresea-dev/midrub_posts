...
jQuery(document).ready( function ($) {
    ...
    ********************************/
     ...
     * @since   0.0.8.3
     */
    Main.load_news_by_page = function (page) {
        
        // Prepare data
        var data = {
            action: 'load_news_by_page',
            page: page
        };

        // Make ajax call
        Main.ajax_call(url + 'user/component-ajax/notifications', 'GET', data, 'display_news_by_page_response');
    ...
    ********************************/ 
    ...
    $( document ).on( 'click', '.notifications-page .notifications-delete-alert', function (e) {
        ...
        // Prepare data
        var data = {
            action: 'delete_news_alert',
            alert: alert_id
        };
        
        // Make ajax call
        Main.ajax_call(url + 'user/component-ajax/notifications', 'GET', data, 'delete_news_alert_response');
    ...
    ********************************/
     ...
     * @since   0.0.8.3
     */
    Main.methods.display_news_by_page_response = function ( status, data ) {
        ...
        if ( status === 'success' ) {
            ...
            // List all alerts news
            for ( var n = 0; n < data.news.length; n++ ) {

                // Unread mark
                var unread = '';

                // Verify if the news alert is unread
                if ( parseInt(data.news[n].page_seen) === 0 ) {
                    unread = ' class="notifications-unread-notification"';
                }
                
                notifications += '<li data-alert="' + data.news[n].alert_id + '"' + unread + '>'
                    + '<div class="row">'
                        + '<div class="col-11">'
                            + '<a href="' + url + 'user/notifications?p=news&alert=' + data.news[n].alert_id + '" class="notifications-show-notification">'
     ...
     * @since   0.0.8.3
     */
    Main.methods.delete_news_alert_response = function ( status, data ) {
