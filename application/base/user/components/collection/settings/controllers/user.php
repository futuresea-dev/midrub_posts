...
class User {
    ...
    private function set_user_view() {
        ...
        if ( $this->CI->input->get('p', true) ) {
            ...
            if ( ($this->CI->input->get('p', true) === 'invoices') && $this->CI->input->get('invoice', true) ) {

                // Require the Invoices Inc
                require_once MIDRUB_BASE_ADMIN_COMPONENTS_ADMIN . 'inc/invoices.php';

                // Get the invoice
                $the_invoice = $this->CI->base_model->get_data_where(
                    'invoices',
                    '*',
                    array(
                        'invoice_id' => $this->CI->input->get('invoice', true),
                ...
                if ( $the_invoice ) {
                    echo $the_invoice[0]['invoice_text'];
                } else {
                    show_404();
                }
