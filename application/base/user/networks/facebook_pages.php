...
class Facebook_pages implements MidrubBaseUserInterfaces\Networks {
    ...
    public function connect() {
        ...
        if ( get_option( 'app_inbox_enable' ) || get_option( 'app_inboxall_enable' ) || get_option( 'app_chatbot_enable' ) ) {
    ...
    public function save($token = null) {
        ...
        if ($this->CI->input->post()) {
            ...
            if ($this->CI->form_validation->run() == false) {
                ...
                // Get user data
                $response = json_decode(get('https://graph.facebook.com/me/accounts?limit=500&access_token=' . $token), true);
                ...
                if ( $net_ids ) {
                    ...
                    if ( isset($response['data'][0]['id']) ) {
                        ...
                        for ( $y = 0; $y < count($response['data']); $y++ ) {
                            ...
                            // Save page
                            if ( $this->CI->networks->add_network('facebook_pages', $response['data'][$y]['id'], $token, $this->CI->user_id, $expires, $response['data'][$y]['name'], '', $response['data'][$y]['access_token']) ) {
                                $check++;
                            }
                    ...
                    echo $this->CI->ecl('Social_login_connect')->view($this->CI->lang->line('social_connector'), '<p class="alert alert-error">' . $this->CI->lang->line('networks_no_pages_selected') . '</p>', false);
                    exit();
            ...
            if ( isset($access_token[0]) ) {
                ...
                if ( isset($response['data'][0]['id']) ) {
                    ...
                    $args = array(
                        ...
                        'network_name' => 'facebook_pages',
                        'items' => $items,
                        'connect' => $this->CI->lang->line('networks_pages'),
