...
class Instagram implements MidrubBaseUserInterfaces\Networks {
    ...
    public function connect() {
        ...
        // Get redirect url
        $loginUrl = $helper->getLoginUrl(site_url('user/callback/instagram'), $permissions);
    ...
    public function save($token = null) {
        ...
        $access_token = $helper->getAccessToken(site_url('user/callback/instagram'));
            ...
            if ( !empty($getUserdata['data']) ) {
                ...
                foreach ( $getUserdata['data'] as $data ) {
                    ...
                    if ( isset($data['instagram_business_account']) ) {
                        ...
                        if ( !$this->CI->networks->check_account_was_added('instagram', $data['instagram_business_account']['id'], $this->CI->user_id) ) {

                            // Add new account
                            if ( $this->CI->networks->add_network('instagram', $data['instagram_business_account']['id'], $access_token[0], $this->CI->user_id, $expires, $data['instagram_business_account']['username'], '') ) {
                                $check++;
                            }
    ...
    public function post($args, $user_id = null) {
        ...
        if ( $args['img'] ) {
            ...
            // Upload photo
            $creation = json_decode(post('https://graph.facebook.com/' . $user_details[0]->net_id . '/media', $photo_params, $token), true);

            // Verify if the photo was uploaded
            if ( isset($creation['id']) ) {

                // Publish photo
                $publish = json_decode(post('https://graph.facebook.com/' . $user_details[0]->net_id . '/media_publish', array('creation_id' => $creation['id']), $token), true);

                // Verify if the photo was published
                if ( isset($publish) ) {
                ...
                // Save the error
                $this->CI->user_meta->update_user_meta( $user_id, 'last-social-error', json_encode($creation) );
