 ...
 * @link     https://www.midrub.com/
 */
class Telegram_groups implements MidrubBaseUserInterfaces\Networks {
    ...
    public function connect() {
        ...
        if ($this->CI->input->post()) {
            ...
            if ($this->CI->form_validation->run() == false) {
                ...
                if ( !empty($data['result']) ) {
                    ...
                    foreach ( $data['result'] as $result ) {
                        ...
                        if ( !isset($result['message']) || !isset($result['message']['chat']['title']) ) {
                            continue;
                        }

                        // Verify if chat_id already exists
                        if ( isset($items[$result['message']['chat']['id']]) ) {
                            continue;
                        }

                        // Set item
                        $items[$result['message']['chat']['id']] = array(
                            'net_id' => $result['message']['chat']['id'],
                        ...
                        if ( in_array($result['message']['chat']['id'], $net_ids) ) {
                            ...
                            // Set as connected
                            $items[$result['message']['chat']['id']]['connected'] = TRUE;
                    ...
                    $args = array(
                        ...
                        'callback' => site_url('user/callback/telegram_groups'),
                        'inputs' => array(
                            array(
                                'api_key' => $api_key
                            )
            ...
            echo $this->CI->ecl('Social_login')->content('Api Key', '', 'Connect', $this->get_info(), 'telegram_groups', '');            
                    ...
                    if ( !empty($data['result']) ) {
                        ...
                        foreach ( $data['result'] as $result ) {
                            ...
                            if ( !$this->CI->networks->check_account_was_added('telegram_groups', $chat_id, $this->CI->user_id) ) {
                                ...
                                // Save group
                                $this->CI->networks->add_network('telegram_groups', $chat_id, $api_key, $this->CI->user_id, '', $title, '', '');
                        ...
                        // Verify if groups were added
                        if ( $count ) {

                            // Display the success message
                            echo $this->CI->ecl('Social_login_connect')->view($this->CI->lang->line('social_connector'), '<p class="alert alert-success">' . $this->CI->lang->line('networks_all_groups_were_connected') . '</p>', true); 
                            exit();
...
/* End of file telegram_groups.php */
