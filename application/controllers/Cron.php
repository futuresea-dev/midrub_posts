...
class Cron extends MY_Controller {
    ...
    public function run() {
        ...
        new MidrubBase\MidrubBase('init');
        ...
        foreach (glob(APPPATH . 'base/user/apps/collection/*', GLOB_ONLYDIR) as $directory) {
            ...
            if (!the_option('app_' .  $app . '_enable')) {
                continue;
            }
        ...
        if ( !the_option('cron_update_verify') ) {

            // Verify for updates
            $this->check_for_updates();

        } elseif ( the_option('cron_update_verify') !== date('Y-m-d') ) {

            // Verify for updates
            $this->check_for_updates();            

        } elseif ( !the_option('cron_subscription_verify') ) {

            // Verify for subscriptions
            $this->check_for_subscriptions();            

        } elseif ( the_option('cron_subscription_verify') !== date('Y-m-d') ) {
    ...
    protected function check_for_updates() {
        ...
        if ( glob(APPPATH . 'base/plugins/collection/*', GLOB_ONLYDIR) ) {
            ...
            foreach (glob(APPPATH . 'base/plugins/collection/*', GLOB_ONLYDIR) as $directory) {
                ...
                // Verify if update already is in the database
                $plugin_update = $this->CI->base_model->get_data_where('updates', '*', array(
                    'slug' => $plugin,
                    'type' => 'plugins'
                ));
    ...
    protected function check_for_subscriptions() {
        ...
        $get_subscriptions = $this->CI->base_model->get_data_where('users_meta', 'users_meta.*, subscriptions.subscription_id', array(
            'users_meta.meta_name' => 'plan_end',
            'users_meta.meta_value <'=> ( time() - 432000 )
        ),
        array(),
        array(),
        array(array(
        ...
        if ( $get_subscriptions ) {
            ...
            foreach ( $get_subscriptions as $get_subscription ) {
                ...
                if ( time() > strtotime($get_subscription['meta_value']) ) {
                    ...
                    // Get user's plan
                    $get_plan = $this->CI->base_model->get_data_where('users_meta', 'users_meta.meta_id,plans.*', array(
                    ...
                    if ( $get_plan ) {
                        ...
                        // Verify if plan is free
                        if ( $get_plan[0]['plan_price'] === '0.00' ) {
                            ...
                            $get_default_plan = $this->CI->base_model->get_data_where('plans', '*', array(
                                'plan_id' => 1
                            ));

                            // Verify if the default plan exists
                            if ( $get_default_plan ) {
                                ...
                                $this->CI->base_model->update(
                                    ...
                                    array(
                                        'meta_value' => date('Y-m-d H:i:s', (time() + ($get_default_plan[0]['period'] * 86400)))
                                    )
                                ); 
