...
class Userarea extends MY_Controller {
    ...
    public function delete_user_account() {
        ...
        // Load Base Model
        $this->load->ext_model( MIDRUB_BASE_PATH . 'models/', 'Base_model', 'base_model' );

        // Get all user medias
        $get_medias = $this->base_model->get_data_where('media', '*', array('user_id' => $this->user_id));
        ...
        if ( $get_medias ) {
            ...
            foreach( $get_medias as $media ) {
                ...
                // Get file path
                $filename = str_replace(base_url(), FCPATH, $media['body']);
