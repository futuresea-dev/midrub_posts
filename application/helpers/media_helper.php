            ...
            if ( $CI->form_validation->run() === false ) {
                ...
                if ( in_array( $type, $check_format ) ) {
                    ...
                    if ( $type === 'image/png' || $type === 'image/jpeg' || $type === 'image/jpg' || $type === 'image/gif' ) {
                        ...
                        file_put_contents(FCPATH . 'assets/share/' . $file_name . '.' . str_replace('image/', '', $type), fopen($link, 'r'));
                        
                        // Set read permission
                        chmod(FCPATH . 'assets/share/' . $file_name . '.' . str_replace('image/', '', $type), 0644); 
                        ...
                        // Save uploaded file data
                        $last_id = $CI->media->save_media($CI->user_id, $CI->config->base_url() . 'assets/share/' . $file_name . '.' . str_replace('image/', '', $type), 'image', $cover, $size);
                ...
                try {
                    ...
                    if ( !file_exists($filename) ) {
                        ...
                        if ( $returns ) {
                            ...
                            echo json_encode(array(
                                'success' => TRUE,
                                'message' => $CI->lang->line('file_deleted_successfully'),
                                'user_storage' => calculate_size($total_storage)
                            ));
